...
const NAV_LINKS = [
  ...
  { label: "Reviews",      href: "#testimonials", sortOrder: 2 },
...
const SERVICE_CARDS = [
  { title: "Regular Wash",        description: "Everyday clothes washed, dried, and neatly folded. Ideal for shirts, trousers, and casual wear.",                price: "From $2/kg",     imageAlt: "Clothes in a washing machine",             accentColor: "#1a7fba", sortOrder: 0 },
  { title: "Dry Cleaning",        description: "Gentle solvent-based cleaning for delicate fabrics — suits, dresses, blazers, and formal wear.",                 price: "From $4/item",   imageAlt: "Dry-cleaned suit on a hanger",             accentColor: "#d97706", sortOrder: 1 },
  { title: "Iron & Press",        description: "Professional steam-ironing for a crisp, wrinkle-free finish on all garments.",                                   price: "From $1/item",   imageAlt: "Steam iron pressing a shirt",              accentColor: "#10b981", sortOrder: 2 },
  { title: "Perfume & Fragrance", description: "Choose from our curated collection of premium fragrances to freshen your laundry.",                              price: "Add-on service", imageAlt: "Collection of fabric softener fragrances", accentColor: "#8b5cf6", sortOrder: 3 },
  { title: "Bedding & Linen",     description: "Duvets, bedsheets, pillowcases, curtains, and tablecloths — washed and fresh every time.",                       price: "From $5/set",    imageAlt: "Freshly washed bed linen",                 accentColor: "#ec4899", sortOrder: 4 },
  { title: "Express Service",     description: "Same-day or next-day turnaround when you need your clothes back in a hurry.",                                     price: "+50% surcharge", imageAlt: "Express delivery clock",                   accentColor: "#0d9488", sortOrder: 5 },
] as const;

const HOW_IT_WORKS_STEPS = [
  { stepNumber: "01", title: "Place Your Order", description: "Use our app or call us. Tell us what you need, choose your service, and pick a convenient collection time.", imageAlt: "Person placing an order on a phone",               accentColor: "#1a7fba", sortOrder: 0 },
  { stepNumber: "02", title: "We Pick It Up",    description: "Our driver arrives at your address at the scheduled time. No waiting around — we come to you.",              imageAlt: "Delivery driver picking up laundry",               accentColor: "#10b981", sortOrder: 1 },
  { stepNumber: "03", title: "We Clean It",      description: "Your laundry is washed, dried, ironed, and quality-checked in our professional facility.",                  imageAlt: "Industrial washing machines in a laundry facility", accentColor: "#8b5cf6", sortOrder: 2 },
  { stepNumber: "04", title: "Delivered Fresh",  description: "Clean, folded, and fragrant — your laundry is delivered back to your door right on schedule.",              imageAlt: "Delivery of clean laundry to front door",          accentColor: "#ec4899", sortOrder: 3 },
] as const;
...
const FOOTER_LINKS = [
  ...
  { column: "quick_links", label: "Reviews",      href: "#testimonials",             sortOrder: 2 },
  { column: "quick_links", label: "Contact",      href: "#contact",                  sortOrder: 3 },
...
async function seedCms() {
  ...
  const [navbar] = await db
    .insert(cmsNavbar)
    .values({
      brandName: "Akiro Laundry",
      logoUrl:   null,
      logoAlt:   "Akiro Laundry logo",
      ctaLabel:  "Book Now",
      ctaHref:   "#contact",
      isActive:  true,
    })
  ...
  if (navbar) {
    await db.insert(cmsNavLinks).values(
      NAV_LINKS.map((l) => ({ ...l, navbarId: navbar.id }))
    ).onConflictDoNothing();
  ...
  const [hero] = await db
    ...
    .values({
      ...
      primaryCtaLabel:   "Order a Pickup",
      primaryCtaHref:    "#contact",
      secondaryCtaLabel: "View Services",
      secondaryCtaHref:  "#services",
      heroImageUrl:      null,
      heroImageAlt:      "Freshly laundered clothes ready for delivery",
      isActive:          true,
    })
    .onConflictDoNothing()
    .returning();
  ...
  if (hero) {
    await db.insert(cmsHeroStats).values(
      HERO_STATS.map((s) => ({ ...s, heroId: hero.id }))
    ).onConflictDoNothing();
  ...
  const [servicesSection] = await db
    ...
    .values({
      badge:    "What We Offer",
      headline: "Every Fabric, Every Need",
      subtext:  "From your daily work shirts to your finest silk gowns — we handle it all with care, precision, and a personal touch.",
      isActive: true,
    })
    .onConflictDoNothing()
    .returning();
  ...
  if (servicesSection) {
    await db.insert(cmsServiceCards).values(
      SERVICE_CARDS.map((c) => ({ ...c, sectionId: servicesSection.id, imageUrl: null }))
    ).onConflictDoNothing();
  ...
  const [howSection] = await db
    ...
    .values({
      ...
      headline: "Laundry Done in 4 Easy Steps",
      subtext:  "We handle everything from collection to delivery so your day stays free.",
      isActive: true,
    })
    .onConflictDoNothing()
    .returning();
  ...
  if (howSection) {
    await db.insert(cmsHowItWorksSteps).values(
      HOW_IT_WORKS_STEPS.map((s) => ({ ...s, sectionId: howSection.id, imageUrl: null }))
    ).onConflictDoNothing();
  ...
  const [testimonialsSection] = await db
    ...
    .values({
      ...
      subtext:         "Don't just take our word for it — here's what our customers say.",
      aggregateRating: "4.9",
      reviewCount:     "2,400+",
      isActive:        true,
    })
  ...
  if (testimonialsSection) {
    await db.insert(cmsTestimonials).values(
      TESTIMONIALS.map((t) => ({
        ...
        avatarUrl: null,
        avatarAlt: `Photo of ${t.authorName}`,
      }))
    ).onConflictDoNothing();
  ...
  const ctaInserted = await db
    ...
    .values({
      ...
      headline:          "Book Your First Pickup",
      headlineAccent:    "Free of Charge",
      ...
      primaryCtaHref:    "tel:+67077230001",
      secondaryCtaLabel: "WhatsApp",
      secondaryCtaHref:  "https://wa.me/67077230001",
      bgImageUrl:        null,
      isActive:          true,
    })
    .onConflictDoNothing()
  ...
  await db
    .insert(cmsContactItems)
    .values(CONTACT_ITEMS.map((c) => ({ ...c, iconUrl: null, isActive: true })))
    .onConflictDoNothing();
  ...
  const [footer] = await db
    ...
    .values({
      brandName:     "Akiro Laundry",
      tagline:       "Premium laundry & dry-cleaning service in Timor-Leste.",
      logoUrl:       null,
      logoAlt:       "Akiro Laundry",
      copyrightText: `© ${new Date().getFullYear()} Akiro Laundry & Perfume. All rights reserved.`,
      isActive:      true,
    })
    .onConflictDoNothing()
    .returning();
  ...
  if (footer) {
    await db
      .insert(cmsFooterLinks)
      .values(FOOTER_LINKS.map((l) => ({ ...l, footerId: footer.id, iconUrl: null, isActive: true })))
      .onConflictDoNothing();
  ...
  console.log("\n💡 Image fields are null — upload via the CMS media manager and update each row.");
  process.exit(0);
}
